...
func TestNonExistingChannelSigning(t *testing.T) {
	sk, _, err := GenerateKeyPair(&Params{n: 32, w: 4, gf: 1, rootH: 4, chanH: 10})
	if err != nil {
...
func TestChannelSigningEnoughSigsInChain(t *testing.T) {
	...
	sk, pk, err := GenerateKeyPair(&Params{n: 32, w: 4, gf: 1, rootH: 3, chanH: 10})
	if err != nil {
		t.Fatalf("keygeneration gave error %s", err)
	}
...
func TestChannelSigningGrowing(t *testing.T) {
	...
	sk, pk, err := GenerateKeyPair(&Params{n: 32, w: 4, gf: 1, rootH: 3, chanH: chanH})
	if err != nil {
		t.Fatalf("keygeneration gave error %s", err)
	}
